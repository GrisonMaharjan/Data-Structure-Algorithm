...
void testSearchAlgorithms(int N) {
    ...
    high_resolution_clock::time_point binaryEnd = high_resolution_clock::now();
    nanoseconds binaryDuration = duration_cast<nanoseconds>(binaryEnd - binaryStart);

    // Measure Interpolation Search time (repeated searches)
    high_resolution_clock::time_point interpolationStart = high_resolution_clock::now();
    for (int i = 0; i < NUM_SEARCHES; i++) {
        interpolationSearch(arr, 0, N - 1, target);
    }
    high_resolution_clock::time_point interpolationEnd = high_resolution_clock::now();
    nanoseconds interpolationDuration = duration_cast<nanoseconds>(interpolationEnd - interpolationStart);
